...
function init() {
  ...
  // MultiplayerGameInterfaceHandler.createProjectile();
  console.log("handler", MultiplayerGameInterfaceHandler);
}
...
function animate() {
  ...
  const deltaTime =
    ...
    MultiplayerGameInterfaceHandler.STEPS_PER_FRAME;

  for (let i = 0; i < MultiplayerGameInterfaceHandler.STEPS_PER_FRAME; i++) {
    MultiplayerGameInterfaceHandler.updateSpheres(deltaTime, worldOctree);
    // MultiplayerGameInterfaceHandler.CheckForNewProjectilesAndAddThemOrUpdateProjectileLaunches();
  }
